document.addEventListener('DOMContentLoaded', function () {
  ...
  function syncReadings() {
    const expanded = readings.every(function (item) { return item.open; });
    all.setAttribute('aria-expanded', String(expanded));
    all.textContent = expanded ? 'CERRAR TODAS ↗' : 'LEER TODAS ↗';
  }
  all.addEventListener('click', function () {
    const open = !readings.every(function (item) { return item.open; });
    readings.forEach(function (item) { item.open = open; });
    syncReadings();
  });
  readings.forEach(function (item) { item.addEventListener('toggle', syncReadings); });
});
